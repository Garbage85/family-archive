...
Обычное обновление читает `PORT` из существующего `deployment.env` и никогда не
возвращает его к 8090. `--port` при update отклоняется с подсказкой. Для осознанной
смены используйте:
...
Новый порт предварительно проверяется по TCP IPv4/IPv6 и systemd sockets. Затем
создаётся backup, переписываются обе копии deployment-конфига и unit, сервис
перезапускается и проверяется по новому URL. Если health check не проходит, старые
config, unit и порт возвращаются, после чего проверяется восстановленный сервис.
